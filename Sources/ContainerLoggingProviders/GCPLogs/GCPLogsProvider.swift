...
public enum GCPLogsLogDriverContract {
    public static let providerIdentity = LogDriverProviderIdentity(
        id: "com.apple.container.logging.providers.gcplogs",
        version: "1",
        kind: .native
    )
        ...
        do {
            return try LogDriverDescriptor(
                ...
                options: [
                    ...
                    option("gcp-log-cmd"),
                    option("gcp-meta-id"),
                    option("gcp-meta-name"),
                    option("gcp-meta-zone"),
                    option("gcp-project"),
                    option("labels"),
                    option(
                        "labels-regex",
            ...
            preconditionFailure("invalid gcplogs contract: \(error)")
        }
    }
...
public typealias GCPLogsEffectTokenGenerating = SplunkEffectTokenGenerating
public typealias RandomGCPLogsEffectTokenGenerator =
    RandomSplunkEffectTokenGenerator

public actor GCPLogsLogDriverProvider: ContainerLogDriverProvider {
    private struct SessionEntry: Sendable {
        let request: LogDriverStartRequestV1
        let token: LogDriverOpaqueEffectTokenV1
        let session: GCPLogsDriverSession
    ...
    private let configurationResolver: any GCPLogsConfigurationResolving
    private let tokenGenerator: any GCPLogsEffectTokenGenerating
    ...
    public init(
        providerGeneration: UInt64 = 1,
        configurationResolver: any GCPLogsConfigurationResolving,
        tokenGenerator: any GCPLogsEffectTokenGenerating =
            RandomGCPLogsEffectTokenGenerator()
    ) {
        self.descriptorValue = GCPLogsLogDriverContract.descriptor(
            providerGeneration: providerGeneration
        )
    ...
    public func start(
        ...
        case .prepared(let started):
            return started
        case .conflict:
            throw GCPLogsProviderError.idempotencyConflict
    ...
    public func openReader(
        ...
        throw GCPLogsProviderError.readUnsupported
    }
    ...
    private func validateProviderIdentity(
        ...
        guard
            ...
            throw GCPLogsProviderError.invalidProviderIdentity
        }
    }
    ...
    private func validate(
        ...
        guard call.effectTokenMaterial.isByteIdentical(to: entry.token) else {
            throw GCPLogsProviderError.invalidEffectToken
        }
        switch call.fence {
        case .candidate(
            ...
            guard
                ...
                throw GCPLogsProviderError.invalidSessionFence
            }
