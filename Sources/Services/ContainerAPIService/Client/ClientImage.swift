...
extension ClientImage {
    private static var serviceIdentifier: String {
        ContainerServiceNamespace.current.imagesServiceIdentifier
    }
...
extension ClientImage {
    ...
    private static func matchDigestPrefix(_ reference: String, in all: [ClientImage]) -> ClientImage? {
        ...
        return Self.uniqueMatch(in: all) { image in
            Self.sha256Identifier(from: image.digest)?.hasPrefix(digestPrefix) == true
        }
    }
    ...
    private static func sha256Identifier(from digest: String) -> String? {
    ...
    private static func matchDisplayName(_ reference: String, in all: [ClientImage], containerSystemConfig: ContainerSystemConfig) throws -> ClientImage? {
        ...
        return Self.uniqueMatch(in: all) { image in
            guard let storedReference = try? Reference.parse(image.reference) else {
                return false
            }
