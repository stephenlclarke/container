    ...
    DockerLoggingSharedResponseBackend
{
    ...
    private static func networkSettingsObject(
        ...
        return [
            ...
            "EndpointID": "",
            "Gateway": first?.ipv4Gateway.map(String.init(describing:)) ?? "",
            "GlobalIPv6Address": first?.ipv6Address.map(cidrAddress) ?? "",
            "GlobalIPv6PrefixLen": first?.ipv6Address.map(cidrPrefixLength) ?? 0,
            "IPAddress": first?.ipv4Address.map(cidrAddress) ?? "",
            "IPPrefixLen": first?.ipv4Address.map(cidrPrefixLength) ?? 0,
    ...
    private static func endpointSettingsObject(
        ...
        [
            ...
            "EndpointID": "",
            "Gateway": attachment.ipv4Gateway.map(String.init(describing:)) ?? "",
            "IPAddress": attachment.ipv4Address.map(cidrAddress) ?? "",
            "IPPrefixLen": attachment.ipv4Address.map(cidrPrefixLength) ?? 0,
