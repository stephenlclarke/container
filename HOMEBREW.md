...
Homebrew formulae install prebuilt release-quality package assets. The main
formula is built from `container` `main` with `containerization` `main`; the
release formula is built from `container` `release` with `containerization`
`release`. The builder shim is not installed as a formula; `container` pins the
