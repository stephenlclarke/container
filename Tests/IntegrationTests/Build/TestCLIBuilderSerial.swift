...
struct TestCLIBuilderSerial {
    ...
    @Test func testBuildSSHForwarding() async throws {
        @Sendable func startUnixSocket(at path: String) -> Int32 {
            ...
            withUnsafeMutableBytes(of: &addr.sun_path) { bytes in
                path.withCString { cStr in
                    bytes.copyMemory(from: UnsafeRawBufferPointer(start: cStr, count: path.utf8.count + 1))
                }
            }
        ...
        try await ContainerFixture.with { f in
            try await f.withBuilder { f in
                ...
                f.addCleanup { try? FileManager.default.removeItem(at: socketDir) }

                let socketPath = socketDir.appendingPathComponent("ssh-auth.sock").path
                let gitSocketPath = socketDir.appendingPathComponent("git-auth.sock").path
                let serverFd = startUnixSocket(at: socketPath)
                f.addCleanup { Darwin.close(serverFd) }
                let gitServerFd = startUnixSocket(at: gitSocketPath)
                f.addCleanup { Darwin.close(gitServerFd) }
                ...
                try f.createContext(
                    ...
                    dockerfile: """
                        ...
                        """)
                let image = "registry.local/ssh-forwarding:\(UUID().uuidString)"
                try f.build(
                    tag: image,
                    contextDir: dir,
                ...
                let namedDir = try f.createTempDir()
                try f.createContext(
                    dir: namedDir,
                    dockerfile: """
                        FROM ghcr.io/linuxcontainers/alpine:3.20
                        ...
                        """)
                let namedImage = "registry.local/ssh-forwarding-named:\(UUID().uuidString)"
                try f.build(
                    tag: namedImage,
                    contextDir: namedDir,
                    otherArgs: ["--ssh", "git=\(socketPath)"])
                try f.assertImageBuilt(namedImage)

                let multiDir = try f.createTempDir()
                try f.createContext(
                    dir: multiDir,
                    dockerfile: """
                        FROM ghcr.io/linuxcontainers/alpine:3.20
                        ...
                        """)
                let multiImage = "registry.local/ssh-forwarding-multiple:\(UUID().uuidString)"
                try f.build(
                    tag: multiImage,
                    contextDir: multiDir,
                    otherArgs: [
                        "--ssh", "default=\(socketPath)",
                        "--ssh", "git=\(gitSocketPath)",
                    ])
                try f.assertImageBuilt(multiImage)
            }
        }
    }
